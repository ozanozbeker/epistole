...
A single Python API for sending email, regardless of the backend behind it.
...
Travelers used them to tell which road led where. The name fits a library whose
job is to take one message and direct it down whichever road you have chosen.

## Credit

`herma` is inspired by [blastula](https://github.com/rstudio/blastula), an R
package for composing and sending email. It is not a port, and the API does not
mirror blastula's.
