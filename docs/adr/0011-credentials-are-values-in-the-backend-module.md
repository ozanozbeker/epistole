# Credentials are values in each backend's module, and Herma builds the vendor object

Every backend takes `credential=`, a frozen value from that backend's own module: `herma.smtp.Password` and `herma.smtp.OAuth`, `herma.graph.ClientSecret`, `herma.graph.Certificate`, and `herma.graph.ManagedIdentity`, `herma.gmail.ServiceAccount` and `herma.gmail.AuthorizedUser`.
...
The value holds inputs only; `connect()` builds the `msal` or `google-auth` object and adapts it to Herma's private token-source shape.
SMTP OAuth takes a Graph or Gmail value and derives its scope from that value's issuer.
Consent flows and token storage stay out of scope, as [#2](https://github.com/ozanozbeker/herma/issues/2) says.
Decided on [#18](https://github.com/ozanozbeker/herma/issues/18).
...
Holding those in a Herma value and building the vendor object inside `connect()` costs Herma about two hundred lines and the caller nothing.

**Values live with the backend, not in one `creds` namespace.**
A single `herma.creds` module was rejected because the values are not interchangeable: a `Password` means nothing to Graph and a `ManagedIdentity` means nothing to SMTP.
Putting each value in the module of the backend that issues its tokens makes the valid set visible at the import.
SMTP OAuth is the one crossover, because an Exchange Online SMTP token comes from the same Entra app that Graph uses and a Gmail SMTP token from the same Google credential Gmail uses, so `herma.smtp.OAuth` takes the other module's value rather than duplicating it.
...
`google-auth` and `msal` do not implement it; Herma adapts both privately, as ADR-0009 already requires.
...
SMTP OAuth needs `https://outlook.office365.com/.default` for a Graph value and `https://mail.google.com/` for a Gmail value, which the value's module already tells Herma.
A foreign `get_token` object has no issuer Herma knows, so `OAuth(..., scope=...)` is required there and `TypeError` otherwise.
...
- **Caller builds the vendor object, Herma adapts.**
  The prototype's shape and #2's literal wording.
  Rejected above: vendor knowledge on every user.
- **One `herma.creds` namespace.**
  Rejected above: hides which values a backend accepts.
- **`herma.microsoft` and `herma.google` modules shared by two backends each.**
  Would spare `herma.smtp` an import from `herma.graph`.
...
- **Accept a bare token or a callable.**
  Rejected above.
- **Herma owns consent flows and storage.**
...
- `Password` is not deprecated anywhere in Herma; it is disabled on one tenant type, and the documentation names the tenant, not the mechanism.
- Herma maintains the input formats of two vendor constructors.
